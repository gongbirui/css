...
class Parser {
  ...
  // 报错
  error(msg) {
    var err = new Error("before line:" + this.lineno + ",column:" + this.column +"  " + msg);
    err.reason = msg;
  ...
  atkeyframes() {
    ...
    if (!m) return this.error("@keyframes missing name");
    var name = m[1];
  ...
  declarations() {
    ...
    var decl;
    while ((decl = this.declaration() || this.rules() || false)) {
